...
test("manifest v6 loads the platform Hybrid addon and exposes its startup diagnostic", () => {
  ...
  assert.match(nativeSource, /await require\("premiere-motion-tracker-0\.4\.3\.uxpaddon"\)/);
...
test("panel assets use the current version cache key", () => {
  ...
  assert.match(index, /premiereBridge\.js\?v=0\.4\.3/);
  assert.match(index, /trajectory\.js\?v=0\.4\.3/);
});
